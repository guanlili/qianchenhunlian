  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
        ...
        <Section title="基本信息">
          <Field label="姓名" value={form.real_name} onChange={update("real_name")} placeholder="张三" />
          <Field label="民族" value={form.ethnicity} onChange={update("ethnicity")} placeholder="汉" />
          <Field label="性别" value={form.gender} onChange={update("gender")} placeholder="男 / 女" />
          <DateField label="出生年月" value={form.birth_date} onChange={update("birth_date")} />
          <Field label="身高 (cm)" value={form.height} onChange={update("height")} placeholder="170" inputMode="numeric" />
          <Field label="体重 (kg)" value={form.weight} onChange={update("weight")} placeholder="65" inputMode="numeric" />
          <Field label="身体状况" value={form.health_status} onChange={update("health_status")} placeholder="健康" />
          ...
          <SelectField
            ...
            placeholder="选择"
          />
          <Field label="性格类型" value={form.personality_type} onChange={update("personality_type")} placeholder="开朗 / 稳重" />
        </Section>

        <Section title="学历 / 职业 / 收入">
          <Field label="学历" value={form.edu} onChange={update("edu")} placeholder="本科" />
          <Field label="专业" value={form.major} onChange={update("major")} placeholder="计算机" />
          ...
          <SelectField
            ...
            placeholder="选择"
          />
          <Field label="兴趣爱好" value={form.hobbies} onChange={update("hobbies")} placeholder="阅读 / 旅游" />
        </Section>
        ...
        <Section title="房 / 车">
          ...
          <FullField>
            ...
            <Input value={form.house_car_loan} onChange={(e) => update("house_car_loan")(e.target.value)} placeholder="房贷剩余 50万 / 无车贷" />
          </FullField>
        </Section>

        <Section title="联系方式 (仅红娘可见)">
          <Field label="手机号" value={form.contact_phone} onChange={update("contact_phone")} placeholder="13800138000" />
          <Field label="微信号" value={form.contact_wechat} onChange={update("contact_wechat")} placeholder="wxid_xxx" />
        </Section>
